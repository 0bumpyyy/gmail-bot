...
// ─────────────────────────────────────────────
const mailingControlMenu = new Menu('mailing-control')
...
accountActionMenu = new Menu('account-action')
    ...
    await ctx.editMessageText("📥 Отправьте `.json` файл.\n\nФормат:\n`[{\"email\":\"user@mail.com\",\"name\":\"John\"}, ...]`", { parse_mode: 'Markdown' });
})
...
gmailMenu = new Menu('gmail-management')
        ...
        range.text(`${status}  ${shortEmail}  ${base}`, async (ctx) => {
            ...
            if (account.recipients) {
                try {
                    ...
                    recipientsInfo = account.csvName || 'Загружены';
                }
            }
            await ctx.editMessageText(`⚙️ *Управление аккаунтом*\n\n📧 \`${account.email}\`\n👥 База: \`${recipientsInfo}\`\n📍 Прогресс: ${account.currentIndex} отправлено`, { parse_mode: 'Markdown', reply_markup: accountActionMenu });
        });
    ...
    const msg = await ctx.reply("📧 Отправьте данные в формате:\n`email@gmail.com пароль_приложения`", { parse_mode: 'Markdown' });
...
// ─────────────────────────────────────────────
const textTemplatesListMenu = new Menu('text-templates-list')
    .dynamic(async (ctx, range) => {
    const userId = String(ctx.from?.id);
...
// ─────────────────────────────────────────────
const textPlatformsUSAMenu = new Menu('text-platforms-usa')
...
// ─────────────────────────────────────────────
const textPlatformsUKMenu = new Menu('text-platforms-uk')
    .text("🎨  Depop", async (ctx) => {
...
// ─────────────────────────────────────────────
const textPlatformsMenu = new Menu('text-platforms')
    .text("🇺🇸  США", async (ctx) => {
    await ctx.editMessageText("🇺🇸 Выберите платформу:", { reply_markup: textPlatformsUSAMenu });
})
    .text("🇬🇧  UK", async (ctx) => {
...
// HTML ШАБЛОНЫ — СПИСОК
// ─────────────────────────────────────────────
const htmlTemplatesListMenu = new Menu('html-templates-list')
    .dynamic(async (ctx, range) => {
    const userId = String(ctx.from?.id);
    const platform = ctx.session.currentPlatform || 'DEPOP_USA';
...
// HTML ШАБЛОНЫ — США
// ─────────────────────────────────────────────
const htmlPlatformsUSAMenu = new Menu('html-platforms-usa')
    .text("🛍  Mercari", async (ctx) => {
    ctx.session.currentPlatform = 'MERCARI_USA';
...
// HTML ШАБЛОНЫ — UK
// ─────────────────────────────────────────────
const htmlPlatformsUKMenu = new Menu('html-platforms-uk')
    .text("🎨  Depop", async (ctx) => {
    ctx.session.currentPlatform = 'DEPOP_UK';
    await ctx.editMessageText("📋 HTML шаблоны — *Depop 🇬🇧*", { parse_mode: 'Markdown', reply_markup: htmlTemplatesListMenu });
})
...
// HTML ШАБЛОНЫ — РЕГИОН
// ─────────────────────────────────────────────
const htmlPlatformsMenu = new Menu('html-platforms')
    .text("🇺🇸  США", async (ctx) => {
    await ctx.editMessageText("🇺🇸 Выберите платформу:", { reply_markup: htmlPlatformsUSAMenu });
})
    .text("🇬🇧  UK", async (ctx) => {
...
// ─────────────────────────────────────────────
const genPlatformsUSAMenu = new Menu('gen-platforms-usa')
    .text("🛍  Mercari", async (ctx) => {
...
// ─────────────────────────────────────────────
const genPlatformsUKMenu = new Menu('gen-platforms-uk')
    .text("🎨  Depop", async (ctx) => {
...
// ─────────────────────────────────────────────
const genPlatformsMenu = new Menu('gen-platforms')
    .text("🇺🇸  США", async (ctx) => {
...
mainMenu = new Menu('main-menu')
    ...
    await ctx.reply(`📊 *Статистика рассылки*\n\n✅ Успешно: *${successCount}*\n❌ Ошибок: *${errorCount}*\n${progressText || '\n_Базы получателей не загружены_'}`, { parse_mode: 'Markdown' });
    await ctx.answerCallbackQuery().catch(() => { });
})
    .row()
    .row()
    ...
    await ctx.reply(user?.token
        ? `🔑 *Токен установлен*\n\n✅ Ваш токен активен\n\nХотите обновить?`
        : `🔑 *Токен не установлен*\n\n❌ Введите ваш токен API`, { parse_mode: 'Markdown' });
    ...
    if (isActive) {
        ...
        range.text("🚀  Запустить рассылку", async (ctx) => {
            ...
            const label = platformLabels[template.platform] || template.platform;
            const chatId = ctx.chat.id;
            await ctx.reply(`🚀 Запуск рассылки\n📌 Платформа: *${label}*\n✉️ Шаблон: *${template.name}*`, { parse_mode: 'Markdown' });
            await ctx.reply("📡 Пульт управления:", { reply_markup: mailingControlMenu });
            await ctx.answerCallbackQuery().catch(() => { });
            runMailing({ delayRange: ctx.session.selectedDelay, userId }, async (logMessage) => {
...
bot.command('start', async (ctx) => {
    ...
    await clearLastBotMessage(ctx);
    await ctx.reply("👋 *Панель управления рассылками*\n\nВыберите действие:", { parse_mode: 'Markdown', reply_markup: mainMenu });
});
// ─────────────────────────────────────────────
...
bot.on('message', async (ctx) => {
    ...
    // ── ТЕКСТОВЫЙ ШАБЛОН ──────────────────────
        ...
        const msg = await ctx.reply(`*Шаг 2 из 4* — Введите тему письма (Subject):\n\n_Название: "${ctx.session.currentTemplateName}"_`, { parse_mode: 'Markdown' });
        ctx.session.lastBotMessageId = msg.message_id;
    }
        ...
        const msg = await ctx.reply(`*Шаг 3 из 4* — Введите текст письма:\n\nДоступные переменные:\n\`{{NAME}}\` — имя получателя\n\`{{LINK}}\` — сгенерированная ссылка\n\`{{ORDER_ID}}\` — случайный номер заказа`, { parse_mode: 'Markdown' });
        ctx.session.lastBotMessageId = msg.message_id;
    }
        ...
        const msg = await ctx.reply(`*Шаг 4 из 4* — Введите имя отправителя:\n\n_Например: Support, John от Depop_`, { parse_mode: 'Markdown' });
        ctx.session.lastBotMessageId = msg.message_id;
    }
    ...
    // ── HTML ШАБЛОН ───────────────────────────
        ...
        const msg = await ctx.reply(`*Шаг 2 из 5* — Введите тему письма (Subject):\n\n_Название: "${ctx.session.currentTemplateName}"_`, { parse_mode: 'Markdown' });
        ctx.session.lastBotMessageId = msg.message_id;
    }
        ...
        const msg = await ctx.reply(`*Шаг 3 из 5* — Введите резервную ссылку (или точку если не нужна):\n\n_Тема: "${ctx.session.htmlSubject}"_`, { parse_mode: 'Markdown' });
        ctx.session.lastBotMessageId = msg.message_id;
    }
        ...
        const msg = await ctx.reply(`*Шаг 4 из 5* — Отправьте файл \`.html\`\n\nВ коде письма используйте:\n\`{{NAME}}\` \`{{LINK}}\` \`{{ORDER_ID}}\``, { parse_mode: 'Markdown' });
        ctx.session.lastBotMessageId = msg.message_id;
    }
    ...
    // ── JSON ЗАГРУЗКА ─────────────────────────
        ...
        try {
            ...
            const msg = await ctx.reply('❌ Неверный формат.\nНужно: `[{"email":"...","name":"..."}, ...]`', { parse_mode: 'Markdown' });
            ctx.session.lastBotMessageId = msg.message_id;
            return;
        }
        ...
        ctx.session.step = 'IDLE';
        await ctx.reply(`✅ Загружено *${valid.length}* получателей из \`${doc.file_name}\``, { parse_mode: 'Markdown', reply_markup: mainMenu });
    }
    // ── РУЧНАЯ ГЕНЕРАЦИЯ ССЫЛКИ ───────────────
        ...
        try {
            ...
            await ctx.reply(`🔗 *Ссылка сгенерирована*\n\n` +
                `🏪 Площадка: *${label}*\n` +
                `👤 Имя: *${name}*\n\n` +
                `📎 Основная:\n\`${fullLink}\`\n\n` +
                `✂️ Короткая:\n\`${shortLink}\`\n\n` +
                `🔍 Поиск:\n\`${searchLink}\``, { parse_mode: 'Markdown', reply_markup: mainMenu });
        }
        catch (err) {
            try {
                await ctx.api.deleteMessage(ctx.chat.id, waitMsg.message_id);
